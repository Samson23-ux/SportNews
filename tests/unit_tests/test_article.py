...
async def test_create_rating(
    ...
    path: str = f"{base_path}.article_service.create_rating"
    with patch(path, new_callable=AsyncMock) as rating:
...
async def test_update_article(
    ...
    path: str = f"{base_path}.article_service.update_article_in_db"
...
async def test_update_rating(
    ...
    path: str = f"{base_path}.article_service.update_rating"
    with patch(path, new_callable=AsyncMock) as rating:
...
async def test_delete_article(
    ...
    path: str = f"{base_path}.article_service.delete_article_from_db"
    with patch(path, new_callable=AsyncMock) as article:
...
async def test_delete_rating(
    ...
    article_id: PydanticObjectId = PydanticObjectId()
    get_article_by_id.return_value = fake_article_with_rating

    delete_path: str = f"{base_path}.article_service.delete_rating_from_db"
    with patch(delete_path, new_callable=AsyncMock) as rating:
...
async def test_delete_article_images(
    ...
    delete_path: str = f"{base_path}.article_service.delete_rating_from_db"
    with patch(delete_path, new_callable=AsyncMock) as images:
